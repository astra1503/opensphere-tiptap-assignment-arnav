...
export const updateCssVariables = (targetNode: HTMLElement, config: PaginationPlusOptions) => {
    ...
    const cssVariables = {
        "page-height": `${config.pageHeight}px`,
        "page-margin-top": `${config.marginTop}px`,
        "page-margin-bottom": `${config.marginBottom}px`,
        "page-margin-left": `${config.marginLeft}px`,
        "page-margin-right": `${config.marginRight}px`,
        "page-content-margin-top": `${config.contentMarginTop}px`,
        "page-content-margin-bottom": `${config.contentMarginBottom}px`,
        "page-gap-border-color": `${config.pageGapBorderColor}`,
        "page-width": `${config.pageWidth}px`,
      }
...
export const getHeaderHeightSelector = (pageNumber: PageNumber, type: "actual" | "content") => {
  return type === "actual" ? `.page-header-${pageNumber}` : `.page-header-${pageNumber} .page-header-content`;
}
...
export const getFooterHeightSelector = (pageNumber: PageNumber, type: "actual" | "content") => {
  return type === "actual" ? `.page-footer-${pageNumber}` : `.page-footer-${pageNumber} .page-footer-content`;
}
...
export function getFooter(footerRightContent: string, footerLeftContent: string, onFooterClick: (event: MouseEvent) => void, pageNumber?: PageNumber) {
  ...
  pageFooter.classList.add(`page-footer-${pageNumber ? pageNumber : 0}`);
  ...
  pageFooterContent.classList.add("page-footer-content");
  ...
  pageFooterLeft.classList.add("page-footer-left");
  pageFooterLeft.innerHTML = footerLeft;

  const pageFooterRight = document.createElement("div");
  pageFooterRight.classList.add("page-footer-right");
...
export function getHeader(headerRightContent: string, headerLeftContent: string, onHeaderClick: (event: MouseEvent) => void, pageNumber?: PageNumber) {
  const pageHeader = document.createElement("div");
      ...
      pageHeader.classList.add(`page-header-${pageNumber ? pageNumber : 0}`);
      ...
      const headerLeft = headerLeftContent.replace(
        "{page}",
        `<span class="page-number-plus"></span>`
      );
...
export const getContentHeight = (editorDom: HTMLElement, pageNumber: PageNumber) => {
  ...
  const pageBreak = editorDom.querySelector('#pages > .page-break:nth-child(' + pageNumber + ') > .page');
  if(pageBreak) {
