  ...
  doc.descendants((node, pos) => {
    if (node.type.name === 'hardBreak') {
      ...
      const widget = Decoration.widget(afterPos, () => {
        ...
        el.classList.add('br-decoration');
        return el;
      });
...
export const PaginationPlus = Extension.create<PaginationPlusOptions, PaginationPlusStorage>({
  ...
  onCreate() {
    ...
    targetNode.classList.add("with-pagination");
    targetNode.style.border = `1px solid var(--page-gap-border-color)`;
    targetNode.style.paddingLeft = `var(--page-margin-left)`;
    targetNode.style.paddingRight = `var(--page-margin-right)`;
    targetNode.style.width = `var(--page-width)`;

    updateCssVariables(targetNode, this.options);

    const style = document.createElement("style");
    style.dataset.paginationStyle = "";

    style.textContent = `
      .pagination-gap{
        border-top: 1px solid;
        border-bottom: 1px solid;
        border-color: var(--page-gap-border-color);
      }
      .with-pagination,
      .with-pagination .first-page-header {
        counter-reset: page-number page-number-plus 1;
      }
      .with-pagination .image-plus-wrapper,
      .with-pagination .table-plus td,
      .with-pagination .table-plus th {
        max-height: var(--page-max-content-child-height);
        overflow-y: auto;
      }
      .with-pagination .image-plus-wrapper {
        overflow-y: visible;
      }
      .with-pagination .page-break {
        counter-increment: page-number page-number-plus;
      }

      .with-pagination .page-break:last-child .pagination-gap {
        display: none;
      }
      .with-pagination .page-break:last-child .page-header {
        display: none;
      }

      .with-pagination table tr td,
      .with-pagination table tr th {
      ...
      .with-pagination table tbody > tr{
        display: table-row !important;
      }
      .with-pagination *:has(>br.ProseMirror-trailingBreak:only-child) {
        display: table;
        width: 100%;
      }
      .with-pagination .br-decoration {
        display: table;
        width: 100%;
      }
      .with-pagination .table-row-group {
        max-height: var(--page-max-content-child-height);
        overflow-y: auto;
        width: 100%;
      }
      .with-pagination .page-footer-left,
      .with-pagination .page-footer-right,
      .with-pagination .page-header-left,
      .with-pagination .page-header-right {
      ...
      .with-pagination .first-page-header .page-header-right{
        margin-right: 0px !important;
      }
      .with-pagination .first-page-header .page-header-left{
        margin-left: 0px !important;
      }
      .with-pagination .page-number::before {
        content: counter(page-number);
      }
      .with-pagination .page-number-plus::before {
      ...
      .with-pagination .page-header{
        padding-bottom: var(--page-content-margin-top) !important;
        padding-top: var(--page-margin-top) !important;
        display: inline-flex;
        justify-content: space-between;
        max-height: calc(calc(var(--page-height) * 0.45) - var(--page-margin-top) - var(--page-content-margin-top));
        overflow-y: hidden;
      }
      .with-pagination .page-footer{
        padding-top: var(--page-content-margin-bottom) !important;
        padding-bottom: var(--page-margin-bottom) !important;
        display: inline-flex;
        justify-content: space-between;
        max-height: calc(calc(var(--page-height) * 0.45) - var(--page-content-margin-bottom) - var(--page-margin-bottom));
        overflow-y: hidden;
      }
    `;
        ...
        view: (editorView: EditorView) => {
          return {
            update: (view : EditorView) => {
                ...
                if(maxContentHeight) {
                  view.dom.style.setProperty(`--page-max-content-child-height`, `${maxContentHeight - 10}px`);
                }
...
const getExistingPageCount = (view: EditorView) => {
  ...
  const paginationElement = editorDom.querySelector("[data-pagination]");
  if (paginationElement) {
...
function createDecoration(
    ...
    (view) => {
      ...
      const pageBreakDefinition = (firstPage: boolean, pageHeader: HTMLElement, pageFooter: HTMLElement, headerHeight: number, footerHeight: number, pageNumber?: PageNumber) => {
        ...
        const marginTop = firstPage
          ...
          : _pageHeight + "px";
        if(pageNumber) {
          page.style.marginTop = `var(--page-content-${pageNumber}, ${marginTop})`;
          }else{
            page.style.marginTop = firstPage
              ? `var(--page-content-first, ${marginTop})`
              : `var(--page-content-general, ${marginTop})`;
          }

        const pageBreak = document.createElement("div");
        pageBreak.classList.add("breaker");
        pageBreak.style.width = `calc(100% + var(--page-margin-left) + var(--page-margin-right))`;
        pageBreak.style.marginLeft = `calc(-1 * var(--page-margin-left))`;
        pageBreak.style.marginRight = `calc(-1 * var(--page-margin-right))`;
      ...
      el.append(fragment);
      el.id = "pages";
      el.classList.add("pages-wrapper");
  ...
  const firstHeaderWidget = Decoration.widget(
    ...
    () => {
      ...
      el.classList.add("first-page-header");
      return el;
    },
    { side: -1 }
  );
